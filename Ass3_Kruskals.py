#function to find parent of a vertex 'i'
def find(parent, i):
    if parent[i] == i:
        return i
    return find(parent, parent[i])

#function to update parent and rank
...
def kruskals_algorithm(graph):
    ...
    #initializing parent and rank for each vertex
    for vertex in graph:
        parent[vertex] = vertex
        rank[vertex] = 0

    #loop to check which edge to include
    for edge in edges:
        ...
        #if both roots are same then it will form a cycle 
        #so add the edge only if they are different
